...
def main():
    ...
    def toca_duas():
        campainha.beep(n=2, on_time=0.2, off_time=0.2)
        
    def comecar_timer3():
        global timer3
        if timer3 == None:
            timer3 = Timer(3.0, toca_duas)
            timer3.start()           
            print("Campainha vai tocar em 3 segundos!")
    ...
    def cancelar_timer3():
        ...
        if timer3 != None:
            ...
            print("Campainha desarmada!")
    
    campainha = Buzzer(16)
    sensor = DistanceSensor(trigger=17, echo=18)
    sensor.threshold_distance = 0.1
    sensor.when_in_range = comecar_timer3
    sensor.when_out_of_range = cancelar_timer3
    ...
    def repetir_print():
        ...
        global timer4
        timer4 = Timer(1.0, repetir_print)
        timer4.start()
    ...
    def salva_data():
        ...
        agora = datetime.now()
        luz = sensor_de_luz.value
        dic_env = {"tempo":agora, "luz":luz}
        print("Simulando envio para o Mongo: ", dic_env)
        
        timer6 = Timer(6.0, salva_data)
        timer6.start()
    ...
    global ultimo_envio
    ultimo_envio = datetime.now() - timedelta(seconds=30)
    
    chave = "*******"
    endereco_base = "https://api.telegram.org/bot" + chave
    id_da_conversa = "*******"
    dados = {"chat_id": id_da_conversa, "text": "O sensor detectou movimento!"}
    ...
    def envia_mov():
        ...
        agora = datetime.now()
        if (agora - ultimo_envio).total_seconds() < 30:
            print("Movimento detectado! Mas ainda não passou 30 segundos...")
        else:
            print("Movimento detectado! Enviando mensagem...")
            resultado = post(endereco_para_mensagem, json=dados)
            ultimo_envio = agora
            print(resultado.text)
    
    sensor_de_movimento.when_motion = envia_mov
